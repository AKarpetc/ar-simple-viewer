...
function __init_joystick_div(div = null) {
	...
	JOYSTICK_DIV.classList.add("joystick_div");
...
var JoyStick = function (attrs) {
	...
	if (attrs.visible) {
		this.__create_fullscreen_div(attrs?.div);
	}
};
...
JoyStick.prototype.__create_fullscreen_div = function (div) {
	if (JOYSTICK_DIV === null) {
		__init_joystick_div(div);
	}
...
var JoyStickButton = function (attrs) {
	...
	if (JOYSTICK_DIV === null) {
		__init_joystick_div(attrs.div);
	}
	this.base = document.createElement('span');
	this.base.innerHTML = this.text;
	/* `div_style` is a variable used to store the style properties of a DOM element. It is used to set
	various CSS styles for the element such as width, height, position, background color, border
	radius, border color, etc. The properties stored in `div_style` are then applied to the
	corresponding DOM element to visually style it on the webpage. */
	div_style = this.base.style;
	div_style.width = this.radius * 2 + 'px';
	div_style.height = this.radius * 2 + 'px';
	div_style.position = 'absolute';
